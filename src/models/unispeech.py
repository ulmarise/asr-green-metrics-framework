...
def main():
    parser = argparse.ArgumentParser(description="Run UniSpeech-SAT ASR model on audio files")
    parser.add_argument("model_id", type=str,
                        default="microsoft/unispeech-sat-base-100h-libri-ft", nargs="?",
                        help="Model ID from HuggingFace (default: microsoft/unispeech-sat-base-100h-libri-ft)")
    ...
    print(f"Loading UniSpeech-SAT model {args.model_id} with {args.precision} precision...")
    ...
    try:
        ...
        for filename in audio_files:
            ...
            try:
                ...
                try:
                    ...
                    for k, v in inputs.items():
                        if isinstance(v, torch.Tensor):
                            inputs[k] = v.to(device).to(torch_dtype)
                ...
                if args.verbose or not transcription.strip():
