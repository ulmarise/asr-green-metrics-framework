...
import torch
from transformers import AutoProcessor, WavLMForCTC, pipeline
import librosa
import os
import sys
import argparse
...
def main():
    parser = argparse.ArgumentParser(description="Run WavLM ASR model on audio files")
    parser.add_argument("model_id", type=str,
                        default="patrickvonplaten/wavlm-libri-clean-100h-base-plus",
                        help="Model ID from HuggingFace (default: patrickvonplaten/wavlm-libri-clean-100h-base-plus)")
    ...
    print(f"Loading WavLM model {args.model_id} with {args.precision} precision...")
    ...
    try:
        ...
        model = WavLMForCTC.from_pretrained(args.model_id, low_cpu_mem_usage=True)
